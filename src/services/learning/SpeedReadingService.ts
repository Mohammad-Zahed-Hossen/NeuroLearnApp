...
// Additional simple interfaces for compatibility
export interface SimpleReadingSession {
  id: string;
  startTime: Date;
  endTime: Date;
  duration: number;
...
export interface SimpleSourceLink {
  id: string;
  url: string;
  title: string;
  description?: string;
...
export interface ReadingPreferences {
  ...
  pauseOnLowComprehension: boolean;
}

// Simple Speed Reading Service Implementation (stub for compatibility)
export class SimpleSpeedReadingService {
  private static instance: SimpleSpeedReadingService;

  public static getInstance(): SimpleSpeedReadingService {
    if (!SimpleSpeedReadingService.instance) {
      SimpleSpeedReadingService.instance = new SimpleSpeedReadingService();
    }
    return SimpleSpeedReadingService.instance;
  }

  private constructor() {}

  async createSession(config: Partial<SimpleReadingSession>): Promise<SimpleReadingSession> {
    const now = new Date();
    const session: SimpleReadingSession = {
  ...
  calculateWPM(wordsRead: number, durationMs: number): number {
