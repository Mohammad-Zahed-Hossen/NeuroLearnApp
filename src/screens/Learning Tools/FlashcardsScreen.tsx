          ...
          <Animated.View style={[styles.cardContainer, { opacity: fadeAnim }]}>
            <GlassCard theme={theme} style={styles.studyCard}>
              <View style={styles.cardHeader}>
                ...
                >
                  {(currentCard.category || 'general').toUpperCase()}
                </Text>
                <Text
                  style={[styles.cardMeta, { color: themeColors.textMuted }]}
                >
                  Interval: {currentCard.interval ?? 0} days • Factor:{' '}
                  {(currentCard.easeFactor ?? 0).toFixed(1)}
                </Text>
              </View>
                        ...
                        try {
                          const d =
                            card.nextReview instanceof Date
                              ? card.nextReview
                              : new Date(card.nextReview as any);
                          return d.toLocaleDateString();
                        } catch (e) {
                          return 'N/A';
                        }
                      })()}{' '}
                      •Interval: {card.interval ?? 0}d •Factor:{' '}
                      {(card.easeFactor ?? 0).toFixed(1)}
                    </Text>
