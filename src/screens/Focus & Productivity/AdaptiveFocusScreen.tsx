...
import { FocusSession } from '../../services/storage/StorageService';
import {
  MindMapGenerator,
  NeuralNode,
...
export const AdaptiveFocusScreen: React.FC<FocusTimerScreenProps> = ({
  ...
  const {
    startFocusSession: startGlobalFocus,
    endFocusSession: endGlobalFocus,
  } = useFocus();
  ...
  useEffect(() => {
    ...
    selectedTarget,
    onNavigate,
    calculateAdaptiveConfiguration,
  ]);
  ...
  const handleLogDistraction = useCallback(async () => {
    try {
      ...
      Alert.alert(
        'Distraction Logged',
        'Distraction has been recorded and neural penalty applied.',
      );
    } catch (error) {
        ...
        <ScreenContainer theme={theme}>
          ...
          >
            <GlassCard theme={theme} style={styles.distractionCard}>
              <Text
                style={[styles.distractionTitle, { color: themeColors.text }]}
              >
                What distracted you?
              </Text>
              ...
              <View style={styles.inputGroup}>
                ...
                >
                  {distractionSeverity === 1 &&
                    'Minor distraction - barely noticed'}
                  {distractionSeverity === 2 &&
                    'Light distraction - quick glance'}
                  {distractionSeverity === 3 &&
                    'Moderate distraction - brief engagement'}
                  {distractionSeverity === 4 &&
                    'Significant distraction - lost focus'}
                  {distractionSeverity === 5 &&
                    'Major distraction - completely derailed'}
                </Text>
              </View>
