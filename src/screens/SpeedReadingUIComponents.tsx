...
const DefaultErrorFallback: React.FC<{ error?: Error; retry?: () => void }> = ({ error, retry }) => (
  <View style={styles.errorContainer}>
    ...
    <Text style={styles.errorMessage}>
      {error?.message || 'An unexpected error occurred in the speed reading component.'}
    </Text>
    {retry && (
...
export const WPMSelector: React.FC<WPMSelectorProps> = ({
  ...
  return (
    <View style={styles.wpmSelector} accessible={true} accessibilityRole="adjustable" accessibilityLabel={`Reading speed selector, current speed ${currentWPM} words per minute`}>
      <Text style={[styles.wpmLabel, { color: themeColors.text }]} accessibilityRole="header">
        Reading Speed: {currentWPM} WPM
      </Text>

      {isAdaptiveDifferent && (
        <Text style={[styles.adaptiveHint, { color: themeColors.accent }]} accessibilityRole="text">
          💡 Suggested: {adaptiveWPM} WPM (based on cognitive load)
        </Text>
      )}

      <View style={styles.wpmControls} accessibilityRole="toolbar" accessibilityLabel="Speed adjustment controls">
        <TouchableOpacity
          ...
          accessibilityLabel={`Decrease speed by 50, new speed ${Math.max(100, currentWPM - 50)} words per minute`}
          accessibilityHint="Double tap to decrease reading speed by 50 words per minute"
        >
          <Text style={[styles.wpmButtonText, { color: themeColors.text }]}>-50</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          accessible={true}
          accessibilityRole="button"
          accessibilityLabel={`Decrease speed by 25, new speed ${Math.max(100, currentWPM - 25)} words per minute`}
          accessibilityHint="Double tap to decrease reading speed by 25 words per minute"
        >
          <Text style={[styles.wpmButtonText, { color: themeColors.text }]}>-25</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          accessible={true}
          accessibilityRole="button"
          accessibilityLabel={`Increase speed by 25, new speed ${Math.min(1000, currentWPM + 25)} words per minute`}
          accessibilityHint="Double tap to increase reading speed by 25 words per minute"
        >
          <Text style={[styles.wpmButtonText, { color: themeColors.background }]}>+25</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          accessible={true}
          accessibilityRole="button"
          accessibilityLabel={`Increase speed by 50, new speed ${Math.min(1000, currentWPM + 50)} words per minute`}
          accessibilityHint="Double tap to increase reading speed by 50 words per minute"
        >
          <Text style={[styles.wpmButtonText, { color: themeColors.background }]}>+50</Text>
        </TouchableOpacity>
      </View>

      {isAdaptiveDifferent && (
        <TouchableOpacity
          style={[styles.adaptiveButton, { backgroundColor: themeColors.accent }]}
        ...
        >
          <Text style={[styles.adaptiveButtonText, { color: themeColors.background }]}>
  ...
  return (
    <View style={styles.rsvpContainer}>
      ...
      <Text style={[styles.progressText, { color: themeColors.text }]}>
        {currentIndex + 1} / {totalWords} ({Math.round((currentIndex / totalWords) * 100)}%)
      </Text>
        ...
        <View style={[styles.quizContainer, { backgroundColor: themeColors.background }]}>
          ...
          <View style={styles.scoreDisplay}>
            ...
            <Text style={[styles.scoreLabel, { color: themeColors.text }]}>
              {Math.round((correctCount / quiz.questions.length) * 100)}% Correct
            </Text>
          </View>
            ...
            {quiz.questions.map((question, index) => {
              ...
              return (
                <View key={question.id} style={styles.resultItem}>
                  <Text style={[styles.questionNumber, { color: themeColors.text }]}>
                    Question {index + 1}
                  </Text>
                  ...
                  <View style={styles.answerComparison}>
                    ...
                    {!isCorrect && (
                      <View style={styles.answerRow}>
                        <Text style={[styles.answerLabel, { color: themeColors.success }]}>
                          Correct Answer:
                        </Text>
                        <Text style={[styles.answerText, { color: themeColors.text }]}>
                    ...
                    {question.explanation && (
                      <Text style={[styles.explanation, { color: themeColors.textSecondary }]}>
                        {question.explanation}
                      </Text>
                    )}
                  </View>
          ...
          <TouchableOpacity
            style={[styles.quizButton, { backgroundColor: themeColors.primary }]}
            onPress={onClose}
          >
            <Text style={[styles.quizButtonText, { color: themeColors.background }]}>
              Continue
            </Text>
          </TouchableOpacity>
        </View>
    ...
    <Modal visible={true} animationType="slide">
      <View style={[styles.quizContainer, { backgroundColor: themeColors.background }]}>
        ...
        <View style={styles.quizControls}>
          <TouchableOpacity
            style={[styles.quizButton, styles.skipButton, { borderColor: themeColors.textSecondary }]}
            onPress={handleSkip}
          >
            <Text style={[styles.quizButtonText, { color: themeColors.textSecondary }]}>
              Skip
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            style={[
              styles.quizButton,
              {
                backgroundColor: answers[currentQuestionIndex] !== undefined 
                  ? themeColors.primary 
                  : themeColors.surface,
              },
            ]}
          ...
          >
            <Text style={[
              ...
              {
                color: answers[currentQuestionIndex] !== undefined 
...
export const ReadingAnalyticsDisplay: React.FC<ReadingAnalyticsDisplayProps> = ({
  theme,
  analytics,
  currentSession,
  onClose,
  ...
  const getDifficultyColor = (difficulty: string): string => {
    switch (difficulty) {
      case 'easy': return themeColors.success;
      case 'medium': return themeColors.warning;
      case 'hard': return themeColors.error;
      case 'technical': return themeColors.accent;
  ...
  return (
    <Modal visible={true} animationType="slide">
      <View style={[styles.analyticsContainer, { backgroundColor: themeColors.background }]}>
        ...
        <ScrollView style={styles.analyticsContent}>
          ...
          <View style={[styles.sessionCard, { backgroundColor: themeColors.surface }]}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                ...
                <Text style={[styles.statLabel, { color: themeColors.text }]}>Avg WPM</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: themeColors.accent }]}>
                  {(analytics.averageComprehension * 100).toFixed(0)}%
                </Text>
                <Text style={[styles.statLabel, { color: themeColors.text }]}>Avg Comp</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: themeColors.success }]}>
                  {analytics.readingStreak}
                </Text>
                <Text style={[styles.statLabel, { color: themeColors.text }]}>Streak</Text>
              </View>
            </View>
          ...
          <View style={[styles.sessionCard, { backgroundColor: themeColors.surface }]}>
            ...
            {Object.entries(analytics.performanceByDifficulty).map(([difficulty, stats]) => (
              <View key={difficulty} style={styles.difficultyRow}>
                ...
                <View style={styles.difficultyStats}>
                  <Text style={[styles.difficultyValue, { color: themeColors.text }]}>
          ...
          <View style={[styles.sessionCard, { backgroundColor: themeColors.surface }]}>
            ...
            <Text style={[styles.bestTimeText, { color: themeColors.accent }]}>
              🕐 {analytics.bestReadingHour}:00 - {analytics.bestReadingHour + 1}:00
            </Text>
            <Text style={[styles.bestTimeHint, { color: themeColors.textSecondary }]}>
...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  return (
    <View style={styles.controlPanel}>
      ...
      >
        <Text style={[styles.controlButtonText, { color: themeColors.background }]}>
          {isActive ? '⏸️ Pause' : '▶️ Play'}
        </Text>
      </TouchableOpacity>
      
      <TouchableOpacity
        style={[styles.controlButton, styles.stopButton, { borderColor: themeColors.error }]}
        onPress={onStop}
      >
      ...
      {onShowAnalytics && (
        <TouchableOpacity
          style={[styles.controlButton, { backgroundColor: themeColors.surface }]}
