...
const LOGIC_TYPES: Array<{
  ...
  },
  {
    ...
    example: 'The sun rose yesterday and today; Therefore, the sun will rise tomorrow',
  },
  {
    id: 'abductive',
    name: 'Abductive',
    description: 'Best Explanation (Most likely cause given the evidence)',
    example: 'The grass is wet; It probably rained (among possible explanations)',
  },
];
...
const DIFFICULTY_LEVELS = [
  { level: 1, name: 'Beginner', color: '#10B981', description: 'Simple logical structures' },
  { level: 2, name: 'Easy', color: '#84CC16', description: 'Basic reasoning chains' },
  { level: 3, name: 'Medium', color: '#F59E0B', description: 'Moderate complexity' },
  { level: 4, name: 'Hard', color: '#F97316', description: 'Complex reasoning' },
  { level: 5, name: 'Expert', color: '#EF4444', description: 'Advanced logical challenges' },
];
...
export const LogicTrainerScreen: React.FC<LogicTrainerScreenProps> = ({
  ...
  useEffect(() => {
    const initializeScreen = async () => {
      try {
        ...
        console.log(`🧠 Logic Trainer initialized: ${dueNodes.length} nodes due for review`);
  ...
  const requestAIFeedback = useCallback(
    async (premise1: string, premise2: string, conclusion: string) => {
      ...
      if (premise1.length < 10 || premise2.length < 10 || conclusion.length < 10) {
        return;
      }
      ...
      try {
        ...
        const localFeedback = evaluateLogicLocally(premise1, premise2, conclusion);
  ...
  const evaluateLogicLocally = useCallback(
    (premise1: string, premise2: string, conclusion: string): AIFeedback['logicFeedback'] => {
      ...
      // Check completeness
      const hasAllFields = premise1.length >= 10 && premise2.length >= 10 && conclusion.length >= 10;
      ...
      const hasLogicalConnectors =
        /\b(therefore|thus|hence|consequently|because|since|if|then)\b/i.test(conclusion);

      if (hasLogicalConnectors) {
        strengths.push('Uses logical connectors effectively');
        score = Math.min(5, score + 1) as 1 | 2 | 3 | 4 | 5;
      } else {
        improvements.push('Add logical connectors (therefore, thus, hence, etc.)');
      }
      ...
      switch (currentExercise.type) {
        case 'deductive':
          reasoning = 'Deductive reasoning requires premises that logically guarantee the conclusion';
          break;
        case 'inductive':
          reasoning = 'Inductive reasoning builds general principles from specific observations';
          break;
        case 'abductive':
          reasoning = 'Abductive reasoning finds the most likely explanation for given evidence';
          break;
      }
  ...
  const handleSubmitExercise = useCallback(async () => {
    if (!currentExercise.premise1.trim() ||
        !currentExercise.premise2.trim() ||
        !currentExercise.conclusion.trim()) {
      Alert.alert('Incomplete Exercise', 'Please fill in all fields before submitting.');
      return;
    }
    ...
    try {
      ...
      setSessionState(prev => ({
        ...
        averagePerformance: (prev.averagePerformance * prev.nodesCompleted + finalScore) / (prev.nodesCompleted + 1),
      }));
  ...
  }, [isReviewMode, currentNodeIndex, dueLogicNodes, loadLogicNodeIntoExercise]);
      ...
      <AppHeader
        ...
        rightComponent={
          <TouchableOpacity onPress={startTrainingSession}>
            <Text style={[styles.sessionButton, { color: themeColors.primary }]}>
              {sessionState.isActive ? `${sessionState.nodesCompleted}/${sessionState.sessionGoal}` : 'Start Session'}
            </Text>
          </TouchableOpacity>
        }
      />
        ...
        >
          ...
          {sessionState.isActive && (
            <GlassCard theme={theme} style={styles.sessionStatusCard}>
              <Text style={[styles.sessionStatusTitle, { color: themeColors.text }]}>
                🧠 Active Training Session
              </Text>
              <View style={styles.sessionStatusRow}>
                <Text style={[styles.sessionStatusText, { color: themeColors.textSecondary }]}>
                  Progress: {sessionState.nodesCompleted}/{sessionState.sessionGoal}
                </Text>
                <Text style={[styles.sessionStatusText, { color: themeColors.textSecondary }]}>
          ...
          {isReviewMode && dueLogicNodes.length > 0 && (
            <GlassCard theme={theme} style={styles.reviewHeaderCard}>
              <Text style={[styles.reviewHeaderTitle, { color: themeColors.warning }]}>
                📚 Review Mode
              </Text>
              <Text style={[styles.reviewHeaderText, { color: themeColors.textSecondary }]}>
          ...
          <GlassCard theme={theme} style={styles.selectionCard}>
            ...
            >
              {LOGIC_TYPES.map((logicType) => (
                <TouchableOpacity
                  key={logicType.id}
                  onPress={() => setCurrentExercise(prev => ({ ...prev, type: logicType.id }))}
                  style={[
                    styles.typeButton,
                ...
                >
                  <Text style={[
                    ...
                    { color: currentExercise.type === logicType.id ? themeColors.primary : themeColors.text },
                  ]}>
                    {logicType.name}
                  </Text>
                  <Text style={[
                    styles.typeButtonDescription,
                    { color: currentExercise.type === logicType.id ? themeColors.primary : themeColors.textSecondary },
                  ]}>
            ...
            <View style={styles.domainSelector}>
              {DOMAINS.map((domain) => (
                <TouchableOpacity
                  key={domain.id}
                  onPress={() => setCurrentExercise(prev => ({ ...prev, domain: domain.id }))}
                  style={[
                    styles.domainButton,
                    currentExercise.domain === domain.id && [
                      styles.domainButtonActive,
                      { backgroundColor: domain.color + '20', borderColor: domain.color },
                    ],
                    { borderColor: themeColors.border },
                  ]}
                >
                  ...
                  <Text style={[
                    styles.domainText,
                    { color: currentExercise.domain === domain.id ? domain.color : themeColors.text },
                  ]}>
                    {domain.name}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles.difficultySelector}>
              {DIFFICULTY_LEVELS.map((diff) => (
                <TouchableOpacity
                  key={diff.level}
                  onPress={() => setCurrentExercise(prev => ({ ...prev, difficulty: diff.level as 1 | 2 | 3 | 4 | 5 }))}
                  style={[
                    styles.difficultyButton,
                    currentExercise.difficulty === diff.level && [
                      styles.difficultyButtonActive,
                      { backgroundColor: diff.color + '20', borderColor: diff.color },
                    ],
                    { borderColor: themeColors.border },
                  ]}
                >
                  <Text style={[
                    ...
                    { color: currentExercise.difficulty === diff.level ? diff.color : themeColors.text },
                  ]}>
                    {diff.level}
                  </Text>
                  <Text style={[
                    styles.difficultyName,
                    { color: currentExercise.difficulty === diff.level ? diff.color : themeColors.textSecondary },
                  ]}>
                    {diff.name}
                  </Text>
                </TouchableOpacity>
          ...
          <GlassCard theme={theme} style={styles.exerciseCard}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setCurrentExercise(prev => ({ ...prev, question: text }))}
                multiline
                numberOfLines={2}
              />
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ref={premise1Ref}
                style={[styles.textInput, styles.premiseInput, {
                  color: themeColors.text,
                  borderColor: currentExercise.premise1 ? themeColors.success : themeColors.border,
                  backgroundColor: themeColors.surface + '50',
                }]}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                style={[styles.textInput, styles.premiseInput, {
                  color: themeColors.text,
                  borderColor: currentExercise.premise2 ? themeColors.success : themeColors.border,
                  backgroundColor: themeColors.surface + '50',
                }]}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ref={conclusionRef}
                style={[styles.textInput, styles.conclusionInput, {
                  color: themeColors.text,
                  borderColor: currentExercise.conclusion ? themeColors.primary : themeColors.border,
                  backgroundColor: themeColors.surface + '50',
                }]}
            ...
            {aiFeedback.logicFeedback.reasoning && (
              <View style={styles.feedbackContainer}>
                <Text style={[styles.feedbackTitle, { color: themeColors.info }]}>
                  🎯 Logic Analysis
                </Text>
                <Text style={[styles.feedbackScore, { color: themeColors.primary }]}>
                  Score: {aiFeedback.logicFeedback.score}/5
                </Text>
                <Text style={[styles.feedbackReasoning, { color: themeColors.textSecondary }]}>
                  {aiFeedback.logicFeedback.reasoning}
                </Text>

                {aiFeedback.logicFeedback.strengths.length > 0 && (
                  <View style={styles.feedbackSection}>
                    <Text style={[styles.feedbackSectionTitle, { color: themeColors.success }]}>
                      ✅ Strengths:
                    </Text>
                    {aiFeedback.logicFeedback.strengths.map((strength, index) => (
                      <Text key={index} style={[styles.feedbackItem, { color: themeColors.textSecondary }]}>
                        • {strength}
                      </Text>
                    ))}
                  </View>
                ...
                {aiFeedback.logicFeedback.improvements.length > 0 && (
                  <View style={styles.feedbackSection}>
                    <Text style={[styles.feedbackSectionTitle, { color: themeColors.warning }]}>
                      🎯 Improvements:
                    </Text>
                    {aiFeedback.logicFeedback.improvements.map((improvement, index) => (
                      <Text key={index} style={[styles.feedbackItem, { color: themeColors.textSecondary }]}>
                        • {improvement}
                      </Text>
                    ))}
          ...
          <View style={styles.submitContainer}>
            <Button
              title={isSubmitting ? 'Processing...' : 'Evaluate & Save Logic Exercise'}
              onPress={handleSubmitExercise}
              variant="primary"
              theme={theme}
              disabled={isSubmitting || !currentExercise.premise1.trim() || !currentExercise.premise2.trim() || !currentExercise.conclusion.trim()}
          ...
          <GlassCard theme={theme} style={styles.exampleCard}>
            <Text style={[styles.exampleTitle, { color: themeColors.text }]}>
              💡 {LOGIC_TYPES.find(t => t.id === currentExercise.type)?.name} Example
            </Text>
            <Text style={[styles.exampleText, { color: themeColors.textSecondary }]}>
